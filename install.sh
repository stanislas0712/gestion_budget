...
SCRIPTS=("setup-ssl.sh" "renew-ssl.sh" "fix-certbot-permissions.sh" "make-executable.sh" "git-pull-safe.sh" "fix-docker-error.sh" "fix-ssl-challenge.sh" "configure-nginx.sh" "fix-django-allowed-hosts.sh" "quick-fix-containerconfig.sh" "fix-csrf-error.sh" "fix-502-bad-gateway.sh" "deploy-web.sh" "deploy-web-safe.sh" "check-templates.sh" "copy-templates-to-volume.sh" "diagnose-template-download.sh")
